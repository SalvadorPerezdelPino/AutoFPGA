...
class DriverFactory():
    def __init__(self, devices_config: dict, compiler: QuartusCompiler, timing_analyzer: TimingAnalyzer):
    ...
    def get(self, driver_name: str) -> DeviceDriver:
        ...
        driver = None
        if driver_type == "cpu":
            driver = CPUDriver(name=driver_name, project_path=project_path, compiler=self.compiler, timing_analyzer=self.timing_analyzer)
        elif driver_type == "asic":
            driver = ASICDriver(name=driver_name, project_path=project_path, compiler=self.compiler, timing_analyzer=self.timing_analyzer)
        else:
